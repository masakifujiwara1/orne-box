...
def callback_dist(data):
    ...
    try:
        ...
        sum_dist += distance1 + distance2
        dist_count += 1

        min_dist = min(min_dist, distance1)
        min_dist = min(min_dist, distance2)

        # # 相対速度を計算
        # relative_velocity = orne_box_velocity - actor2_velocity

        # # TTCを計算
        # if np.linalg.norm(relative_velocity) > 1e-6:  # ゼロ除算回避
        #     ttc = distance / np.linalg.norm(relative_velocity)
...
def finish_navigation(data):
    global start_time, navigation_duration, navigation_finished, total_distance, sum_dist, dist_count
    if start_time is not None and data.data and not navigation_finished:
        ...
        avg_dist = sum_dist / dist_count if dist_count > 0 else 0.0
        rospy.loginfo("Navigation finished. Duration: %.3f seconds", navigation_duration)
        rospy.loginfo(f'TIME: {navigation_duration:.3f} sec, DISTANCE: {total_distance:.3f} m, MIN_DIST: {min_dist:.3f} m, AVG_SPEED: {avg_speed:.3f} m/s, AVG_DIST: {avg_dist:.3f} m, MIN_TTC: {min_ttc:.3f} s')
